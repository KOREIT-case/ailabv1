...
| 기록 범위 | **자동 실패 3종 + 사용자 신고** (그 외 성공 답변은 저장 안 함) | 저장량·프라이버시 부담 최소화. 단 "틀리게 답한 것"은 기계가 못 잡아 사람이 넣는 경로를 따로 둠 → §5-2-a |
...
### 5-2-a. ★ 네 번째 원인 — `사용자신고` (자동 판별의 사각지대)

위 3종은 전부 **"답을 못 한"** 경우다. 근거를 갖춰 그럴듯하게 **틀리게** 답하면
어떤 기계 판별에도 안 걸린다. 환각 절대 금지가 최우선 가치인데 자동 판별만으로는
이 구멍이 안 막히므로, 화면의 **'이 답변 이상해요'** 버튼으로 사람이 넣는 경로를 둔다.

| | 질문 원문 | 답변 |
|---|---|---|
| 자동 3종 (못 답한 질문) | 전문 저장 | 앞 4000자 |
| **`사용자신고`** (틀리게 답한 질문) | 전문 저장 | 앞 4000자 + **신고 사유** |
| 그 외 정상 답변 | 저장 안 함 | 저장 안 함 |

- **동작**: 버튼 → 사유 입력칸(**선택** — 안 써도 접수된다. 문턱을 낮게 둬야 실제로 눌린다)
  → `POST /report`. 사유는 `answer_head` 맨 앞에 `【신고 사유】` 로 붙어 `--id` 에서 먼저 보인다.
- **버튼이 붙는 위치**: **실제로 답한 답변에만.** 거절·오류는 이미 자동 기록되므로 중복이다.
- **`top_hits`**: 검색 상위가 아니라 **답변이 인용한 근거**가 들어간다(`cited:1` 표시).
  신고는 답변이 나간 뒤에 오므로 그때의 검색 결과가 서버에 남아 있지 않다.
  검토할 때는 어차피 그 질문을 다시 던져 현재 검색 결과를 새로 본다.
- **UPSERT 규칙**: **사용자 신고가 자동 분류를 이긴다.** 사람이 누른 신호를 나중에 들어온
  자동 기록이 덮어써서 지우면 안 된다(실측 확인: 신고 후 같은 질문을 자동 실패시켜도
  `사용자신고` 유지).
- **정렬**: 검토 목록에서 신고 건이 맨 위로 온다
  (`ORDER BY (reason='사용자신고') DESC, hit_count DESC`).
  대전제상 '못 답한 것'보다 **'틀리게 답한 것'이 더 급하다.**
- **상태 추가**: `프롬프트보정` — 자료도 검색도 맞는데 모델이 잘못 해석한 경우.
  신고 건에서 주로 나온다.
...
| 답변 신고(§5-2-a) | 접수·미인증차단·빈질문거부·운영자제외 정상. 신고가 자동분류를 이김 |
