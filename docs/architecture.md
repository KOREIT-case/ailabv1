...
- [x] 청크↔벡터 연결을 순번(id)에서 **안정 키(`파일명#조문`)** 로 전환.
      순번 방식은 corpus에 파일 하나만 추가돼도 뒤 청크가 전부 밀려 엉뚱한 벡터를
      가리켰고, 에러 없이 조용히 검색 순위만 오염됐다. 이제 KV의 키 매니페스트
      (`corpus-vector-keys`)로 이름을 맞춰 찾고, 매니페스트가 없거나 개수가 어긋나면
      벡터를 통째로 버리고 BM25로 폴백한다. 아직 임베딩 안 된 새 청크는 BM25만 적용.
