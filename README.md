...
- 운영 실적 측정(GA4) 계측 완료 — 이용 건수·질의 유형·근거없음 비율·응답시간을
  수집한다. 질문 원문은 보내지 않고 유형만 보낸다. 측정 ID(`GA_ID`)를
  `chatbot/worker/wrangler.toml` 에 넣고 배포하면 켜지며, 비워두면 꺼진 상태다.
  → `docs/ga4-운영실적-가이드.md`
